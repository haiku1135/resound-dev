...
const Email = ({ email }: EmailProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof EmailSchema>) => {
    ...
    startTransition(async () => {
      try {
        ...
        toast.success("メールアドレス変更に必要なURLを記載したメールを送信しました。");
  ...
  return (
    <div>
      <div className="font-bold text-xl text-center mb-10">メールアドレス変更</div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-bold">新しいメールアドレス</FormLabel>
                <FormControl>
                  <Input 
                    type="email"
